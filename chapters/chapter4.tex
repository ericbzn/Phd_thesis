...
Applying the Schwartz's inequality for the integrals on the right-hand side of \mbox{Eq.\ \eqref{eq:time_bandwidth_disp}} we obtain
...
We can normalize the two above expressions following the same reasoning as in the 1-d case. We apply the maximum value condition Eq.\ \eqref{eq:maximun_condition} and the constant spectrum condition Eq.\ \eqref{eq:constant_spectrum_condition} described in section \ref{subsec:filter_normalization} to get $\max{|G(u,v)|} = 1$ and \mbox{$\int_{-\infty}^{\infty} \int_{-\infty}^{\infty} |g(x,y)| dx dy = 1$} for a filter on any frequency $f$ and orientation $\theta$. 
