...
\begin{definition}[Subgraph]
	A subgraph $\mathcal{G}'= (\mathsf{V}', \mathsf{E}')$ is a (partial) graph of $\mathcal{G}=(\mathsf{V}, \mathsf{E})$ if $\mathsf{V}' \subseteq \mathsf{V}$, $\mathsf{E}' \subseteq E$ and $\mathsf{e}_{i,j} \in E'$ $\Rightarrow$ $\mathsf{v}_{i}, \mathsf{v}_{j} \in \mathsf{V}'$.
\end{definition}
...
%replacing the 1 by the edge weight $\omega_{ij}$ if $i, j$ are adjacent.
\end{definition}
...
\begin{definition}[Spanning Tree (ST)]
...
%\begin{enumerate}
% \item \textbf{Watershed-based.} Based on the watershed algorithm, these methods generally depend on an image pre-processing step and definition of the markers. 
% \item \textbf{Density-based.} Perform mode-seeking in a computed density image. These methods usually do not offer control over the number of superpixels or their compactness (they are considered over-segmentation algorithms). 
% \item \textbf{Graph-based.} Treat the image as an undirected graph and do the image partition based on edge-weights  (computed as color differences or similarities).
% \item \textbf{Clustering-based.} Inspired in classical clustering methods (k-means), these techniques group the pixels from seed pixels using color information, spatial information, and other (such as depth information). The number of superpixels and their compactness is controllable.
%\end{enumerate}
...
Normalizing the individual pixels or superpixels' texture signatures leads to Gabor energy information loss, so the classic EMD cannot be applied. We use instead the EMD proposed by \citep{Pele.Werman:ECCV:2008}, which is defined as 
...
%\sum_{j}f_{ij} \leq P_i \, , \; \sum_{i}f_{ij} \leq Q_j \, , \; \sum_{i,j}f_{ij} = \mathrm{min} \left( \sum_{i} P_i , \sum_{j} Q_j\right)\, , \; f_{ij} \geq 0
\end{eqnarray}
...
Spectral clustering is a technique used to cluster elements of a graph \citep{Ng.Jordan.ea:NIPS:2001}. Considering the RAG weighted with the EMD over the Gabor feature space, we used this technique to group the superpixels into perceptual regions consistent with the color and texture information.
...
\multicolumn{1}{|c|}{Mean Shift \citep{Comaniciu.Meer:PAMI:2002}}  & 0.63        & 0.66       & 0.54       & 0.64        & \textbf{0.68}       & 0.56       \\
\multicolumn{1}{|c|}{EGB \citep{Felzenszwalb.Huttenlocher:IJCV:2004}}   & 0.58        & 0.62       & 0.53       & 0.61        & 0.64       & 0.56       \\
\multicolumn{1}{|c|}{NCuts \citep{Cour.Benezit.ea:CVPR:2005}}       & 0.62        & 0.66       & 0.43       & 0.64        & \textbf{0.68}       & 0.45       \\
\multicolumn{1}{|c|}{Canny \citep{Canny:PAMI:1986} }       & 0.58        & 0.62       & 0.58       & 0.60        & 0.63       & 0.58       \\
\multicolumn{1}{|c|}{Pb \citep{Malik.Belongie.ea:IJCV:2001}}          & \textbf{0.65}        & -          & -          & -           & -          & -          \\ \hline
...
\multicolumn{1}{|c|}{Mean Shift \citep{Comaniciu.Meer:PAMI:2002}}  & 0.54         & 0.58         & 0.66         & 0.79              & 0.81              & 1.85               & 1.64              \\ \hline
\multicolumn{1}{|c|}{EGB \citep{Felzenszwalb.Huttenlocher:IJCV:2004}}   & 0.52         & 0.57         & 0.69         & 0.80              & 0.82              & 2.21               & 1.87              \\ \hline
...
We report in Table \ref{table:boundary_scores} the boundary detection scores on the BSDS300 and the BSDS500. Besides, Fig.\ \ref{fig:pr_curves} displays the precision-recall curves of different methods only for the BSDS500. Table \ref{table:segmentation_scores} presents region benchmarks on the BSDS500. 
...
\begin{figure}[!ht]
	\centering
	\includegraphics[width=0.8\textwidth]{pr_curves_mymethod_ANN}
